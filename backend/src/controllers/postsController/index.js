...
class postController {
  ...
  static async sendPost(req, res) {
    ...
    try {
      ...
      if (!selectedUser)
        return res.status(400).send("Não foi possível executar esta ação!");
      const authorization = jwt.verify(
        req.body.token,
        process.env.SECRET_TOKEN
      );
      if (!authorization)
        return res.status(403).send(`Não foi possível enviar o post ${error}`);
  ...
  static async listPost(req, res) {
    try {
      const posts = await postModel
        ...
        .populate({
          path: "autor",
          select: "name email avatar createdDate age",
        })
        .exec();
  ...
  static async editPost(req, res) {
    ...
    try {
      ...
      const findPost = await postModel
        .findOne({ _id: id })
        .populate({ path: "autor", select: "_id name email createdDate age" })
        .exec();
      if (!findPost)
        return res.status(400).send("Não foi possível concluir essa ação!");
      const findUser = await userModel.findOne({ email: req.body.email });
      if (!findUser)
        return res.status(403).send("Não foi possível concluir essa ação!");
      if (findPost.autor._id == findUser._id)
        return res
          .status(403)
          .send("Você não tem permissão para fazer esssa ação!");
  ...
  static async listPostsByUserID(req, res) {
    ...
    try {
      const posts = await postModel
        ...
        .populate({
          path: "autor",
          select: "name email createdDate avatar age",
        })
        .exec();
      for (let i in posts) {
