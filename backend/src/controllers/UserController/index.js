...
class userController {
  ...
  static async recoverPassword(req, res) {
    if (!req.body.email)
      return res
        .status(400)
        .send(
          "Não foi possível recuperar a senha, pois o e-mail não foi informado!"
        );
    try {
      const userSelected = await userModel.findOne({ email: req.body.email });
      if (userSelected.cpf != req.body.cpf)
        return res
          .status(400)
          .send(
            "Não foi possível recuperar a senha, pois a idade não confere!"
          );
      const cryptPassword = bcrypt.hashSync(req.body.password, saltKey);
  ...
  static async listOneUser(req, res) {
    ...
    if (!id)
      return res
        .status(400)
        .send(
          "Não foi possível listar o usuário, pois o id não foi informado!"
        );
    try {
  ...
  static async listOneEmail(req, res) {
    ...
    if (!token)
      return res.status(400).send("Não foi possível executar essa ação");
    try {
      const authorization = jwt.verify(token, process.env.SECRET_TOKEN);
      if (!authorization)
        return res
          .status(403)
          .send("Não foi possível editar os dados do usuário!");
      const userSelected = await userModel.findOne({ email: req.body.email });
      const userData = {
        _id: userSelected._id,
        name: userSelected.name,
  ...
  static async habilitarPerfil(req, res) {
    try {
      ...
      res
        .status(200)
        .send(req.body.habilitado === true ? "Habilitado!" : "desabilitado");
    } catch (error) {
      res.status(400).send("Houve um erro: " + error);
    }
  }
  ...
  static async deleteUser(req, res) {
    ...
    if (!id)
      return res
        .status(400)
        .send(
          "Não foi possível deletar o usuário, pois o id não foi informado!"
        );
    try {
      const authorization = jwt.verify(
        req.body.token,
      ...
      if (!authorization)
        return res.status(403).send("Não foi possível deletar o usuário!");
      if (!req.body.token)
        return res.status(403).send("Não foi possível deletar o usuário!");
      if (req.body.token) {
  ...
  static async listAllUsers(req, res) {
    try {
      const userSelected = await userModel
        .find()
        .populate({ path: "_id", selected: "_id name age email createdDate" })
        .exec();
