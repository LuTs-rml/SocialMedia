...
class commentsController {
  ...
  static async listPostComments(req, res) {
    const { id } = req.params;
    try {
      const postComments = await commentsModel
        .find({ postId: id })
        .populate({ path: "users posts" })
        .exec();
  ...
  static async createComment(req, res) {
    ...
    if (!token)
      return res.status(403).send("Não foi possível fazer esse comentário");
    try {
      const selectedUser = await userModel.findOne({ email: req.body.email });
      if (!selectedUser)
        return res.status(400).send("Não foi possível fazer esse comentário");
